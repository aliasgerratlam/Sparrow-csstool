/* ─────────────────────────────────────────────────────────────────────────
   LZString (compress/decompress to EncodedURIComponent) — URL-safe, no deps.
   Ported verbatim from the original prototype; behaviour is unchanged.
───────────────────────────────────────────────────────────────────────── */
...
function getBaseValue(alphabet: string, character: string): number {
  ...
  if (!dic) {
    dic = {}
    for (let i = 0; i < alphabet.length; i++) dic[alphabet.charAt(i)] = i
    baseReverseDic[alphabet] = dic
  }
  return dic[character] ?? 0
}
...
export function decompressFromEncodedURIComponent(
  ...
  return _decompress(normalized.length, 32, (index) =>
    getBaseValue(keyStr, normalized.charAt(index)),
  )
}
